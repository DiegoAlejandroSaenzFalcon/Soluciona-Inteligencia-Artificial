...
if (!process.env.DISABLE_WHATSAPP) {
  if (process.env.WHATSAPP_TRANSPORT === 'cloud') {
    try {
      const { createTransport } = require('./src/whatsapp/cloud/transport.js');
    ...
    try {
      const { iniciarWhatsApp } = require('./transports/whatsapp.js');
...
try {
  const { iniciarLimpiezaSesiones } = require('./core/db-sqlite.js');
