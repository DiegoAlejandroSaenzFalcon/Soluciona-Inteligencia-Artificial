...
async function initializeDatabase() {
        ...
        if (codes.includes('*')) {
            ...
            for (const perm of allPerms) {
                await db.insert(schema.rolePermissions)
                    .values({ role: role, permissionId: perm.id })
                    .onConflictDoNothing();
            }
        }
        else {
            for (const code of codes) {
                const perm = await db.select({ id: schema.permissions.id })
                    ...
                    .where((0, drizzle_orm_1.eq)(schema.permissions.code, code))
                    .limit(1);
                if (perm.length) {
                    await db.insert(schema.rolePermissions)
                        .values({ role: role, permissionId: perm[0].id })
                        .onConflictDoNothing();
                }
            }
        }
    ...
    const defaultTenant = await db.select().from(schema.tenants).where((0, drizzle_orm_1.eq)(schema.tenants.id, 'default')).limit(1);
    if (!defaultTenant.length) {
        await db.insert(schema.tenants).values({
            id: 'default',
            nombre: config_1.config.negocio || 'Mi Negocio',
            segmento: config_1.config.segmento || 'comidas',
            ciiu: config_1.config.ciiu || '',
            activo: true,
        });
    ...
    const adminUser = await db.select().from(schema.users)
        .where((0, drizzle_orm_1.eq)(schema.users.email, 'admin@localhost'))
        .limit(1);
    if (!adminUser.length) {
        const adminPass = process.env.ADMIN_PASSWORD || crypto_1.default.randomBytes(12).toString('hex');
        const passwordHash = await bcryptjs_1.default.hash(adminPass, 12);
    ...
    const accountsCount = await db.select({ count: (0, drizzle_orm_1.sql) `count(*)` }).from(schema.accounts).where((0, drizzle_orm_1.eq)(schema.accounts.tenantId, 'default'));
...
async function closeDatabase() {
    const { closeDb } = await Promise.resolve().then(() => __importStar(require('./index')));
    await closeDb();
}
