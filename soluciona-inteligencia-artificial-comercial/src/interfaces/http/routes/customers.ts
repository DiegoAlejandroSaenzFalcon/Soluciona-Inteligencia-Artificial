...
const customerRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    ...
    const [rows, countResult] = await Promise.all([
      require('../../../src/db/connection').getClient().unsafe(
        `SELECT * FROM customers ${whereClause} ORDER BY ultima_compra DESC NULLS LAST, nombre ASC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`,
        [...params, limit, offset]
      ),
      require('../../../src/db/connection').getClient().unsafe(
  ...
  fastify.post('/', {
    ...
    await c.unsafe(
      `INSERT INTO customers (id, tenant_id, nombre, telefono, email, direccion, lat, lng, dias_credito, limite_credito, saldo_pendiente, activo, notas, tags, creado_at, actualizado_at)
  ...
  fastify.put('/:id', {
    ...
    if (body.direccion !== undefined) { updates.push(`direccion = $${paramIndex++}`); values.push(body.direccion); }
    if (body.lat !== undefined) { updates.push(`lat = $${paramIndex++}`); values.push(body.lat); }
    if (body.lng !== undefined) { updates.push(`lng = $${paramIndex++}`); values.push(body.lng); }
    if (body.diasCredito !== undefined) { updates.push(`dias_credito = $${paramIndex++}`); values.push(body.diasCredito); }
    if (body.limiteCredito !== undefined) { updates.push(`limite_credito = $${paramIndex++}`); values.push(body.limiteCredito); }
    if (body.notas !== undefined) { updates.push(`notas = $${paramIndex++}`); values.push(body.notas); }
    if (body.tags !== undefined) { updates.push(`tags = $${paramIndex++}`); values.push(JSON.stringify(body.tags)); }
    ...
    await require('../../../src/db/connection').getClient().unsafe(
      `UPDATE customers SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1}`,
      values
    );
  ...
  fastify.get('/top', {
    ...
    const by = request.query.by as string || 'totalGastado';
    ...
    const rows = await require('../../../src/db/connection').getClient().unsafe(
      `SELECT * FROM customers WHERE tenant_id = $1 AND activo = true ORDER BY ${orderBy} DESC LIMIT $2`,
      [user.tenantId, limit]
    );
  ...
  fastify.get('/deuda', {
    ...
    const rows = await c.unsafe(
      'SELECT * FROM customers WHERE tenant_id = $1 AND saldo_pendiente > 0 AND activo = true ORDER BY saldo_pendiente DESC',
      [user.tenantId]
    );
