...
const productRoutes = async (fastify) => {
    ...
    fastify.get('/', {
        ...
        const [rows, countResult] = await Promise.all([
            require('../../../src/db/connection').getClient().unsafe(`SELECT * FROM products ${whereClause} ORDER BY nombre ASC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`, [...params, limit, offset]),
            require('../../../src/db/connection').getClient().unsafe(`SELECT COUNT(*)::int as total FROM products ${whereClause}`, params)
        ]);
        return {
            data: rows,
    ...
    fastify.post('/', {
        ...
        await require('../../../src/db/connection').getClient().unsafe(`INSERT INTO products (id, tenant_id, nombre, codigo, descripcion, precio, costo, categoria, ingredientes, imagen_url, activo, maneja_stock, stock_minimo, stock_maximo, unidad_medida, codigo_barras, impuestos, creado_at, actualizado_at)
    ...
    fastify.put('/:id', {
        ...
        if (body.ingredientes !== undefined) {
            ...
            values.push(body.ingredientes);
        }
        if (body.imagenUrl !== undefined) {
            updates.push(`imagen_url = $${paramIndex++}`);
            values.push(body.imagenUrl);
        }
        if (body.activo !== undefined) {
            updates.push(`activo = $${paramIndex++}`);
            values.push(body.activo);
        }
        if (body.manejaStock !== undefined) {
            updates.push(`maneja_stock = $${paramIndex++}`);
            values.push(body.manejaStock);
        }
        if (body.stockMinimo !== undefined) {
            updates.push(`stock_minimo = $${paramIndex++}`);
            values.push(body.stockMinimo);
        }
        if (body.stockMaximo !== undefined) {
            updates.push(`stock_maximo = $${paramIndex++}`);
            values.push(body.stockMaximo);
        }
        if (body.unidadMedida !== undefined) {
            updates.push(`unidad_medida = $${paramIndex++}`);
            values.push(body.unidadMedida);
        }
        if (body.codigoBarras !== undefined) {
            updates.push(`codigo_barras = $${paramIndex++}`);
            values.push(body.codigoBarras);
        }
        if (body.impuestos !== undefined) {
            updates.push(`impuestos = $${paramIndex++}`);
            values.push(body.impuestos);
        }
        if (updates.length === 0) {
        ...
        await c.unsafe(`UPDATE products SET ${updates.join(', ')} WHERE id = $${paramIndex} AND tenant_id = $${paramIndex + 1}`, values);
        return { ok: true };
    });
