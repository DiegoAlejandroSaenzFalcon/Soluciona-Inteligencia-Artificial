...
const logger = (0, logger_1.getLogger)('auth-routes');
...
const createUserSchema = zod_1.z.object({
    ...
    role: zod_1.z.enum(['admin', 'operador', 'cocina', 'domiciliario', 'contador', 'asesor', 'solo_lectura']),
...
const updateUserSchema = zod_1.z.object({
    ...
    telefono: zod_1.z.string().optional(),
    role: zod_1.z.enum(['admin', 'operador', 'cocina', 'domiciliario', 'contador', 'asesor', 'solo_lectura']).optional(),
...
const authRoutes = async (fastify) => {
    ...
    fastify.post('/login', {
        ...
        const result = await (0, index_1.login)(body.email, body.password, {
            ip: request.ip,
        ...
        const cookie = `refresh_token=${result.refreshToken}; HttpOnly; Path=/; SameSite=Lax; Secure=${(0, config_1.getConfig)().environment === 'production'}; Max-Age=${30 * 24 * 60 * 60}`;
    ...
    fastify.post('/verify-2fa', {
        ...
        const result = await (0, index_1.verifyTwoFactor)(body.twoFactorToken, body.code, {
            ip: request.ip,
            userAgent: request.headers['user-agent'],
        });
        if (result.error) {
    ...
    fastify.post('/refresh', {
        ...
        const result = await (0, index_1.refresh)(body.refreshToken, {
            ip: request.ip,
            userAgent: request.headers['user-agent'],
        });
        if (result.error) {
    ...
    fastify.post('/logout', async (request, reply) => {
        ...
        await (0, index_1.logout)(refreshToken);
    ...
    fastify.post('/change-password', {
        ...
        const userRows = await c.unsafe('SELECT password_hash FROM users WHERE id = $1', [user.id]);
        if (!userRows.length) {
    ...
    fastify.get('/permissions', {
        ...
        const user = request.user;
        const perms = await (0, index_1.getPermissionsForUser)({
            id: user.id,
            tenant_id: user.tenantId,
            email: user.email,
            role: user.role,
    ...
    fastify.post('/enable-2fa', {
        ...
        const c = require('../../../src/db/connection').getClient();
        await c.unsafe('UPDATE users SET two_factor_enabled = true, two_factor_secret = $1 WHERE id = $2', [body.secret, request.user.id]);
        return { ok: true };
    });
    ...
    fastify.post('/disable-2fa', {
        ...
        await c.unsafe('UPDATE users SET two_factor_enabled = false, two_factor_secret = NULL WHERE id = $1', [user.id]);
        return { ok: true };
    });
    ...
    fastify.post('/api/users', {
        ...
        try {
            const rows = await c.unsafe(`INSERT INTO users (tenant_id, email, password_hash, nombre, telefono, role, activo, must_change_password)
         VALUES ($1, $2, $3, $4, $5, $6, $7, 1)
         RETURNING id, tenant_id, email, nombre, telefono, role, activo, two_factor_enabled, must_change_password`, [user.tenantId, body.email.toLowerCase().trim(), passwordHash, body.nombre, body.telefono || null, body.role, body.activo !== false]);
    ...
    fastify.put('/api/users', {
        ...
        if (body.password) {
            const passwordHash = await require('../../../src/auth/index').hashPassword(body.password);
            await c.unsafe('UPDATE users SET password_hash = $1, nombre = COALESCE($2, nombre), telefono = COALESCE($3, telefono), role = COALESCE($4, role), activo = COALESCE($5, activo) WHERE id = $6', [passwordHash, body.nombre || null, body.telefono || null, body.role || null, body.activo == null ? null : body.activo, Number(body.id)]);
        }
        else {
            await c.unsafe('UPDATE users SET nombre = COALESCE($1, nombre), telefono = COALESCE($2, telefono), role = COALESCE($3, role), activo = COALESCE($4, activo) WHERE id = $5', [body.nombre || null, body.telefono || null, body.role || null, body.activo == null ? null : body.activo, Number(body.id)]);
        }
        return { ok: true };
    });
};
